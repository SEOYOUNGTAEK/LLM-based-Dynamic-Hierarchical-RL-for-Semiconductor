...
from agents.llm_agent import LLMAnalyst_Phase1 as LLMStrategicAnalyst  # 조건 (C): Phase 1 개선 (90% 제약 + 단일 메트릭)
...
if __name__ == '__main__':
    model_type = "Baseline_B_Phase1LLM"  # 조건 (C): Phase 1 개선 — 90% 제약 + 단일 메트릭
    ...
    priority_rewards = []
    all_episode_results = []
    all_xai_records = []  # XAI: LLM 의사결정 기록
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            while plantsim_manager.get_value("WaitingSchedule"):
                ...
                # Pure DRL과 동일한 보상 사용 (LLM은 action masking으로만 개입)
                ...
                # eval 에피소드에서는 학습 없음 (수정된 보상으로 buffer 추가)
                if not is_eval:
        ...
        # 조건 (C): Phase 1 LLM 동적 거버넌스 (90% 제약 + 단일 메트릭)
        episode_history.append(episode_summary)
        if not is_eval:
            logger.info(f"[L2-C] Episode {episode + 1} finished. Phase1 LLM analysis...")
            try:
                ...
                logger.info(f"[L2-C] Received {len(new_rules)} rules. Reasoning: {xai_rec.get(chr(39)+chr(114)+chr(101)+chr(97)+chr(115)+chr(111)+chr(110)+chr(105)+chr(110)+chr(103)+chr(39),chr(39)+chr(39))[:80]}")
                safety_agent.set_rules(new_rules)
                all_xai_records.append(xai_rec)
            except Exception as e:
                logger.error(f"[L2-C] Failed.", exc_info=True)
        else:
