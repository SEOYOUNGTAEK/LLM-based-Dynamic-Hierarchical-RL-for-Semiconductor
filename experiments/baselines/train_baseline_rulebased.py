...
from agents.llm_agent import LLMAnalyst_RuleBased as LLMStrategicAnalyst  # 조건 (E): Rule-based mock
...
if __name__ == '__main__':
    ...
    _parser.add_argument('--seed', type=int, default=RANDOM_SEED)
    RANDOM_SEED = _parser.parse_args().seed
    model_type = "RuleBased_LLM"      # (E) Rule-based mock LLM (no Ollama, deterministic)
    np.set_printoptions(suppress=True, precision=6)
    version_no = datetime.now().strftime('%Y%m%d_%H%M%S')
    folder_name = f"output/{version_no}_{model_type}_s{RANDOM_SEED}_abl"
    log_dir = "./" + folder_name
