...
# train_ablation_random.py
# Ablation 2: LLM vs Random Rules
#
# 제안 모델(LLM-HRL)과 동일한 구조(Hard Mask + 동적 갱신)를 유지하되,
# LLM 룰 생성 대신 매 에피소드 랜덤 룰을 생성한다.
#
# 비교 목적: "LLM의 domain-aware 판단이 중요한가,
#            아니면 constraint 구조 자체가 중요한가?"
#
# 사용법: python train_ablation_random.py --seed 42
...
from llm_agent_ablation import RandomRuleAgent
...
CONDITION          = "Ablation_RandomRules"
...
if __name__ == '__main__':
    ...
    # ── 에이전트 초기화 ─────────────────────────────────────────
    # RandomRuleAgent: LLM 없이 매 ep 랜덤 룰 생성 (동일 sanitizer 통과)
    llm_analyst  = RandomRuleAgent(n_rules=2)
    ...
    for episode in range(num_episodes):
        ...
        # 랜덤 룰도 매 에피소드 갱신 (동적 갱신 구조 동일 유지)
        if not is_eval:
            try:
                ...
                logger.error(f"[RandomRule] Failed to update rules: {e}", exc_info=True)
