...
if __name__ == '__main__':
    ...
    reward_weight = 1.0  # 초기 가중치
    original_reward_sum = 0  # 가중치 적용 전 순수 보상 합계
    strategy_count = 0  # 가중치 변경 횟수 추적
    ...
    for episode in range(num_episodes):
        ...
        total_reward = 0
        original_reward_sum = 0  # ⬅️ 이 부분을 반드시 추가하여 매 에피소드 0부터 시작하게 하세요.
        total_target = 0
        total_urgent_lot = 0
        total_lot_move = 0
        ...
        while True:
            ...
            while plantsim_manager.get_value("WaitingSchedule"):
                ...
                # [Baseline B 수정] L1 마스킹을 무시하고 None으로 설정
                # action_mask = safety_agent.get_action_mask(state_summary)  <- 기존 로직 무시
                action_mask = None
                ...
                # Action 결정 시 mask=None 전달
                action, epsilon = get_action(state, prev_epsilon, mask=None)
                ...
                # 가중치 적용 전 순수 보상 누적 (지표 수집용)
                original_reward_sum += reward

                # LLM이 결정한 가중치 적용 (비정상성 주입)
                modified_reward = reward * reward_weight
                total_reward += modified_reward  # 학습 곡선에 반영될 수정된 보상

                # 수정된 보상을 버퍼에 추가하여 학습 안정성 저해 확인
                if not is_first_iteration:
                    dqn.buffer.add((state, action, modified_reward, next_state))
        ...
        if ((episode + 1) % performance_frequency == 0) or (episode == 0) or (episode > num_episodes - 2):
            ...
            episode_summary = {
                ...
                'original_reward': original_reward_sum  # 순수 환경 보상
            }
            L1_strategy_switch_count = 0
        ...
        if iteration_log:
            ...
            try:
                ...
                # 전략적 개입이 필요하다고 판단(규칙 생성) 시 가중치 대폭 상향
                if len(new_rules) > 0:
                    reward_weight = 5.0  # 긴급 물량 처리 강조를 위한 보상 증폭
                    strategy_count += 1
                else:
                    reward_weight = 1.0

                logger.info(f"[L2 Baseline B] Reward weight updated to: {reward_weight}")

            except Exception as e:
                logger.error(f"[L2] Weight update failed: {e}")
