# train_ablation_soft.py
# Ablation 1: Hard Mask vs Soft Reward
#
# 제안 모델(LLM-HRL)과 동일한 LLM 룰 생성 파이프라인을 유지하되,
# L1의 action mask 대신 reward penalty를 reward에 더해 DRL을 간접 유도한다.
#
# 비교 목적: "hard constraint(금지)가 tail failure 제거의 핵심인가,
#            아니면 soft guidance(패널티)로도 같은 효과가 나오는가?"
#
# 사용법: python train_ablation_soft.py --seed 42
...
from llm_agent import LLMStrategicAnalyst
from llm_agent_ablation import SoftRewardAdapter
...
if __name__ == '__main__':
    ...
    soft_adapter = SoftRewardAdapter()   # Hard mask 대신 penalty 계산기
    ...
    for episode in range(num_episodes):
        ...
        episode_penalty_count = 0   # soft penalty가 실제 발동된 횟수
        ...
        while True:
            ...
            while plantsim_manager.get_value("WaitingSchedule"):
                ...
                # state_summary: rule 판단에 사용 (mask 없음 — soft reward만 적용)
                state_summary = plantsim_manager.get_state_summary()

                # Soft reward: mask 없이 DRL이 자유롭게 action 선택
                action, epsilon = dqn.select_action(
                    state, episode, mask=None, greedy=is_eval)
                ...
                # ── Soft Reward: 룰 위반 시 penalty를 reward에 합산 ──
                penalty = soft_adapter.get_penalty(
                    action, state_summary, safety_agent.current_rules)
                if penalty != 0.0:
                    episode_penalty_count += 1
                reward += penalty
        ...
        # ── L2 룰 갱신 (Soft: 동일한 LLM 룰 생성, mask 대신 penalty로 사용) ──
        if not is_eval:
            try:
                ...
                safety_agent.set_rules(new_rules)   # 룰 저장 (penalty 계산에 사용)
                all_xai_records.append(xai_rec)
            except Exception as e:
                logger.error(f"[L2] Failed to update strategy: {e}", exc_info=True)
